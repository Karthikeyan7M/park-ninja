...
let type = document.getElementById("size");
...
let typeErrors = document.getElementById("sizeError");
...
print.style =
  " background: yellow; border: 0; font-size: 11px; font-weight: 600; line-height: 2.5;  outline: transparent; padding: 0 1rem; text-align: center;color:black;border-radius:7px;";
...
alert(`only ${5 - counter} spaces left`);
function createTable() {
  ...
  if (/\s/.test(pla)) {
    //to make sure the plate number inserted doesn't have spaces
  ...
  let ty = document.getElementById("size").value;
  if (ty == "") {
    typeErrors.innerHTML = `<i class="bi bi-exclamation-circle"></i> Car type must be filled out`;
    typeErrors.style = `color: red;font-size: 11px;margin-top:0.3em`;
    return;
  }
  if (ty != "") {
    typeErrors.innerHTML = ``;
  }
  ...
  if (counter < 5) {
    alert(
      `only ${5 - Number(counter)} spaces left, click ok to book your slot `
    );
    ...
    cell6.innerHTML = date.toUTCString(); //toUTCString()  automatically converts the ouput of new Date() into UTC string format
    ...
    let arrivalTimeinMinutes =
      hours * 60 + minutes + Math.floor(secondsinMinutes);
    ...
    remover.style =
      " background: red; border: 0; font-size: 11px; font-weight: 600; line-height: 2.5;  outline: transparent; padding: 0 .3rem;padding-left: .9rem;padding-right: .9rem; text-align: center;color:black;border-radius:7px;";
    //code above assign varies attributes to the new button we created
    cell7.appendChild(remover); //to append it to the Action column in parking table
    clear();
    ...
    remover.onclick = function removeRow() {
      alert("verify the link sended to your mobile");
      counter-=1;
      ...
      function createReceiptTable() {
        ...
        let currrentTimeinMinutes =
          currenthours * 60 +
          currentminutes +
          Math.floor(currentSecondsinMinutes);
        ...
        view.value = "VIEW";
        view.style =
          " background: royalblue; border: 0; font-size: 11px; font-weight: 600; line-height: 2.5;  outline: transparent; padding: 0 .3rem;padding-left: .8rem;padding-right: .8rem; text-align: center;color:black;border-radius:7px;margin-right:.2rem;";
        ...
        removerR.style =
          " background: red; border: 0; font-size: 11px; font-weight: 600;padding-left: .9rem;padding-right: .9rem; line-height: 2.5;  outline: transparent; padding: 0 .3rem; text-align: center;color:black;border-radius:7px;margin-top:0.3em;";
        ...
        let receiptDate =
          receiptDateandTime.getDate() +
          "/" +
          receiptDateandTime.getMonth() +
          "/" +
          receiptDateandTime.getFullYear(); //to convert the the date of parking receipt being issued into dd/mm/yyyy format
  ...
  for (var i = 0; i < plateArray.length - 1; i++) {
    //this for loop checks if the plate number inputed already exists and deletes the whole row if it does
  ...
  //Clears the form input boxes after successfull submission
